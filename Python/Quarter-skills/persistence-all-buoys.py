#
"""
/Python/Quarter-skills/persistence-all-buoys.py
by LIU Duan

This programs is modified from my earlier program quarter-skills-correct.py

Purpose: 
1. To calculate the persistence of each forecast for 3 month.
	The previous program was to calculate the model skills for 3 month. 
2. Make the shift, amptitude, and range adjustment, 
3. Recalculate the skills, and hope to improve.
4. Calculate the skills with the observation at the model starting time, a straight line. 

Steps:
1. Determine one buoy at a time.
2. Get 3 months observation data.
	Change Observation to along-shore (al_), and cross shore (cr_) 
3. Do fft to filter the 40 low pass. 
4. Make it a hourly series.
5. Calculate the skill value of each forecasts.

Calculate persistence skills from the starting point of observation, model, and corrected model.
...
t_model = source_CDF.variables['valid_time'][:,:,:] - 3600*12			# [9, 368, 73], to correct the model time 0.
u_model = source_CDF.variables['water_u'][:,:,:]
v_model = source_CDF.variables['water_v'][:,:,:]
al_model = zeros((9,368,73))

al_skills = zeros((9, 368))			along shore skills
per_al_skills = zeros((9, 368))		persistence along shore skills
al_hourly_obs_start  = zeros(2425)				the first number in the al_hourly_obs, for persistence calculation.
...
per_ -- persistence
fin_ -- final corrected.

s_mean -- serial mean, for the de-mean before fft, and add mean after fft.
...
def skill(obs, t_obs, model, t_model, climate):
    skill_top = 0
    skill_bottom = 0
    mm = 0
    ...
    for n in range(len(t_model)):
        ...
        for m in range(mm, mmm):
            ...
            if t_model[n] == t_obs[m]:
                ...
                skill_bottom = (obs[n]- climate)**2 + skill_bottom
                # print skill_bottom
                mm = m
                break
...
######################### Define variables ##############
al_skills = zeros((9, 368))				# skills for models
obs_per_skills = zeros((9, 368))			# observation persistence skills using the first value of observation 
mod_per_skills = zeros((9, 368))			# model persistence skills using the first value of model
corrected_per_skills = zeros((9, 368))		# corrected model persistence skills using the first value of corrected model 
...
for buoy_number in 0, 2, 3, 4, 5, 6, 8:        				# should look like above.
...
# u_fft and v_fft are the arrays of fft products 

    ###### de-mean ####
    s_mean = mean(al_obs)
    al_obs = al_obs - s_mean
    print "The mean is: ", s_mean, "; ", 
    ###### end of de-mean ####
	
    amp_before_fft = mean(absolute(al_obs))			# amplitude before fft
	
    al_fft = scipy.fft(al_obs) 

    for i in range(len(al_obs)):
        if i<= 60: 				# should be 120 for 40hr low pass
    ...
    # al_discard = scipy.ifft(al_fft).real * 0.0

    al_net = al_obs - al_discard
	
    amp_after_fft = mean(absolute(al_net))			# amplitude after fft
    print "amp_before_fft/amp_after_fft = ", amp_before_fft/amp_after_fft
	
    al_net = amp_before_fft/amp_after_fft * al_net - np.mean(al_net) 			# used to be 2, depend on the fft cut off value.

    ###### add mean back ####
    al_net = al_net + s_mean
    ###### end of add mean back ####
...
## dimensions: buoys = 9; forecast_files = 368; valid_hours = 73 
    ...
    amp_model_before_fft = mean(absolute(al_ln_model))			# amplitude before fft
    ...
    amp_model_after_fft = mean(absolute(al_net_model))			# amplitude after fft
    al_net_model = amp_model_before_fft/amp_model_after_fft * al_net_model - np.mean(al_net_model) 
    print "model amplitude factor: ", amp_model_before_fft/amp_model_after_fft
    ...
    if buoy_number >= 0:
        ...
        # print 'al_net_model.shape: ', al_net_model.shape	
	
        # """	
        fig = pl.figure(buoy_number)
        ax = fig.add_subplot(111)

        line_1 = pl.plot(t_net_model/(3600*24.0)-15240, al_net_model, "r-")  
        line_2 = pl.plot(t_hourly_obs/(3600*24.0)-15240, al_hourly_obs, "b-")

        pl.grid(True)
        ax.set_title( 'Buoy ' + Buoy_names[buoy_number] + '. model-Red; observation-Blue.' )

        pl.xlabel('days')
        pl.ylabel('Along Shore Velocity, m/s')
...
## dimensions: buoys = 9; forecast_files = 368; valid_hours = 73 
    ...
    t_model = source_CDF.variables['valid_time'][:,:,:] - 3600*12				# why? to match the model and the observation
    u_model = source_CDF.variables['water_u'][:,:,:]
    v_model = source_CDF.variables['water_v'][:,:,:]
...
######################################     Skills for each model forecast    ###################################
    ...
    climate = mean(al_hourly_obs)
    # print climate, al_hourly_obs
    for n in range(22, 368):							# when cut data., default: 22, 367
        al_skills[buoy_number,n] = skill(al_hourly_obs, t_hourly_obs, al_model[buoy_number,n,:], t_model[buoy_number,n,:]+ 12*3600, climate)
		


#####################################      Try to draw each forcast on the the figure   #########################
        line3 = pl.plot((t_hourly_obs-1317902400.0)/(3600*24)+ 13.5, al_hourly_obs, "b-")
		
		
        one_forecast_mean = mean(al_model[buoy_number,180,:])
        one_forecast_fft = scipy.fft(al_model[buoy_number,180,:]- one_forecast_mean)
        for i in range(len(one_forecast_fft)):
            if i>= 2: 			### should be 56 for 40 hr low pass, calculate the part to be discarded instead of the part to keep
                one_forecast_fft[i] = 0
	   
        one_forecast_after_fft = scipy.ifft(one_forecast_fft).real + one_forecast_mean
		
		
        # line5 = pl.plot((t_model[buoy_number,166,:]-1317902400.0)/(3600*24)+ 14, one_forecast_after_fft, "k-")
		
		
		
		
        one_forecast = al_model[buoy_number,166,:]
        line4 = pl.plot((t_model[buoy_number,166,:]-1317902400.0)/(3600*24)+ 14, one_forecast, "g-")
        # pl.plot((t_model[buoy_number,n,1]-  132535440)/(24*3600), al_model[buoy_number,n,1], "g-") 
        # print 't_model[buoy_number,n,0] =', t_model[buoy_number,n,0]
        # exit
...
######################################     Persistence Skills for each model forecast    ##############################
    ####### using the starting value of the observation.
    # should be the starting value against the observation
    # obs_per_skills = zeros((9, 368))			# observation persistence skills using the first value of observation 
    # print 'al_hourly_obs.shape: ', al_hourly_obs.shape	
    obs_skill_is_better = 0
    mod_skill_is_better = 0
    al_hourly_obs_start = zeros(2425)
    al_hourly_obs_start = al_hourly_obs_start + al_hourly_obs[0] 
    ...
    # print al_hourly_obs_start							
    for n in range(22, 368):							# when cut data., default: 22, 367
        obs_per_skills[buoy_number,n] = skill(al_hourly_obs, t_hourly_obs, al_hourly_obs_start, t_hourly_obs, climate)
        if obs_per_skills[buoy_number,n] > al_skills[buoy_number,n]:
            obs_skill_is_better = obs_skill_is_better + 1
        if obs_per_skills[buoy_number,n] < al_skills[buoy_number,n]:
            mod_skill_is_better = mod_skill_is_better + 1
    ####### using the starting value of model.
    # should be the starting value against the observation
    # mod_per_skills = zeros((9, 368))			# model persistence skills using the first value of model	
    # print 'al_hourly_obs.shape: ', al_hourly_obs.shape	
    al_mod_start = zeros(2425)					
    for n in range(22, 368):							# when cut data., default: 22, 367
        al_mod_start = al_mod_start + al_model[buoy_number,n,0]		# al_model[buoy_number,n,:], n -- forecast file number, then hours
        mod_per_skills[buoy_number,n] = skill(al_hourly_obs, t_hourly_obs, al_hourly_obs_start, t_hourly_obs, climate)
		
    """
    ###### using the starting value of corrected model.
    # should be the starting value against the observation
    # corrected_per_skills = zeros((9, 368))	
    # print 'al_hourly_obs.shape: ', al_hourly_obs.shape	
    al_hourly_obs_start = zeros(2425)
    al_hourly_obs_start = al_hourly_obs_start + al_hourly_obs[0] 
    # print al_hourly_obs_start							
    for n in range(22, 368):							# when cut data., default: 22, 367
        per_al_skills[buoy_number,n] = skill(al_hourly_obs, t_hourly_obs, al_hourly_obs_start, t_hourly_obs, climate)
    """
...
# skills: 		# defined before loop.
    # shift_skill = zeros((9, 368))
    # range_skill = zeros((9, 368))
    # ampt_skill = zeros((9, 368))

# Amount of correction: 				# defined before loop.
    # A_index_local_offset = zeros((9,368))
    # Ranges = zeros((9,368))
    # amptitude = zeros((9,368))
...
# find the past 5 days for shift, amplitude, and range
    ...
    ## generating past 5 days serial for model
        ...
        ampt_skill[buoy_number, forecast_file_number] = (skill(al_hourly_obs, t_hourly_obs, al_ampt_model, 
            t_model[buoy_number, forecast_file_number, :], climate))
        ...
        shift_skill[buoy_number, forecast_file_number] = skill(al_hourly_obs, t_hourly_obs, al_shift_model, t_shift_model, climate)
        ...
        range_skill[buoy_number, forecast_file_number] = skill(al_hourly_obs, t_hourly_obs, al_range_model, t_shift_model, climate)
        ...
        ampt_skill[buoy_number, forecast_file_number] = skill(al_hourly_obs, t_hourly_obs, al_ampt_model, t_shift_model, climate)
    ...
    print "Persistence skills from the observation, model:", mean(obs_per_skills[buoy_number,22:]), 
    print mean(mod_per_skills[buoy_number,22:]) 
    print "obs_skill_is_better (persistence is better: ) ", obs_skill_is_better, "; mod_skill_is_better = ", mod_skill_is_better
    print "std(obs_per_skills[buoy_number,22:]): ", std(obs_per_skills[buoy_number,22:]), 
    print "; std(al_skills[buoy_number,22:]): ", std(al_skills[buoy_number,22:])
