...
function Load_kml(){
	...
	var anim_file="http://tabs1.gerg.tamu.edu/tglo/testpages/ADCP/anim_file.php";
...
function turn(){	
   ...
   ge.getOptions().setFlyToSpeed(1);
    ...
    lookAt.setLatitude(27.8897);
    lookAt.setLongitude(-94.03703);
	...
	if (tilt_angle <= 0){
    	tilt_increment = +0.5;
		fly_to(27.8897, -94.03703, 10000000, 150*Math.pow(range_factor, 0.47), 0.25, 10, 0);
		setTimeout(function(){
			fly_to(27.8897, -94.03703, 0, 1500000*Math.pow(range_factor, 0.47), 0.25, 0, 0);
			}, 7000); 
		// setTimeout(function(){
			// fly_to(27.8897, -94.03703, 1000000, 150*Math.pow(range_factor, 0.47), 0.25, 50, turn_heading+240);
			// }, 10000); 
...
function turn_control(){		// start or stop turn()
    ...
    var turning_chkBox 	 = document.getElementById('turn_html');
   	if (turning_chkBox.checked){
		...
		fly_to(27.8897, -94.03703, -50, 200*range_factor, 0.6, 65*Math.pow(range_factor, 0.3), 0);
		// sleep(2000);
		turning_obj = setTimeout("turn()", 5000); 
			...
			fly_to(27.8897, -94.03703, -50, 200*range_factor, 0.6, 65*Math.pow(range_factor, 0.3), 0);    
			// fly_to(lat,  lon,  	, altitude, range,     speed, tilt, heading)
			setTimeout("fly_to(27.8897, -94.03703, -50, 100*range_factor, 0.6, 75, 0)", 5000);  
  			}	// end of else.
...
function initCallback(object) {			// 888888888888888888888888888888888888888888888888888888888888888888888888888888888
	...
	setTimeout("fly_to(27.8897, -94.03703, -50, 200*range_factor, .5, 65*Math.pow(range_factor, 0.3), 0)", 22000); 
		// Buoy N above water.

	// fly_to(lat,    lon,  , altitude, range,                               	speed, 	tilt  				, heading)
	// fly_to(27.889715, -94.0370483, 0, 500*Math.pow(range_factor, 0.47), 0.2, 65*Math.pow(range_factor, 0.3), 180);
	// sleep(10000); 
