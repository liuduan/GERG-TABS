...
<tr BGCOLOR="#99CCFF" class="HEADING-STYLE"><td align="right">Checkout: </td><td><input type="text" name="checkout" /><br> </td>
...
(The Checkout Number Must Exactly Match The Buoy Depolyment Record!!)
...
$strSQL = "SELECT * FROM instrument_table ". $_SESSION["instrument_site"]. " ". $_GET['order'];				# Change table name here.
// echo $strSQL;
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");				# $objQuery is the Resource
?>


<form name="myform" action="instrument.php" method="get">
Buoy Site Selection 
...
if ($_SESSION['ordinal'] == "ASC") $_SESSION['ordinal'] = "DESC"; else $_SESSION['ordinal'] = "ASC";
...
<table border="1">
  <tr BGCOLOR="#99CCFF">
    <th> <form action="instrument.php" method="get" style="display: inline; vertical-align:top;">	
...
<input type="submit" name="mysubmit" value="old_checkout" /></form></th>
    ...
    <th> <form action="instrument.php" method="get" style="display: inline; vertical-align:top;">	
<input type="hidden" id="age" name="order" value="ORDER BY ordinal <?php echo $_SESSION['ordinal'];?>" />
...
$extra_row_number = $last_row["Buoy_checkoutumber"] + 1;
...
Buoy_checkoutumber: <input type="text" name="Buoy_checkoutumber" />
<input type="submit" name="mysubmit" value="Insert New Record" />
</form>
--!>
